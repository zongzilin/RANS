PROGRAM RANS_MPI
    ...
    !     ! U - Velocity solve
        ...
        !call MPI_SEND_RECV(Pc,nhy,nhx,ll,lh,left,right,tag,cart_comm,req)
        CALL MPI_waitall(4,req,stat,ierr)
        ...
        DO i = 2,nhy - 1
            DO j = ll,lh
                Unew(i,j)= Unew(i,j)-(dt/rho)*(Pc(i+1,j)-Pc(i-1,j))/(2*hx)
                Vnew(i,j)= Vnew(i,j)-(dt/rho)*(Pc(i,j+1)-Pc(i,j-1))/(2*hy)
            end DO
        end DO
